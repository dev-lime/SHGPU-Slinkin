/*
 Даны однобайтовые значения X и Y. Создать программу, определяющую сколько потребуется последовательных операций вида X=X+Y и X=X-Y для перехода через границу типа переменной X. Рассмотреть все варианты сочетаний типов signed char и unsigned char для переменных X и Y.
*/
